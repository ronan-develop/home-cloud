...
if [[ "$UPDATE_MODE" == true ]]; then
    ...
    # ── Build Tailwind en local (le binaire ne tourne pas sur o2switch) ────────
    title "── Build Tailwind (local) ──────────────────"
    info "Compilation du CSS Tailwind en local…"
    php bin/console tailwind:build --minify
    success "Tailwind compilé → var/tailwind/app.built.css"

    # ── Envoi du CSS compilé sur le serveur ────────────────────────────────────
    info "Envoi de app.built.css sur le serveur…"
    scp ${SSH_KEY_OPTS:+-i ${SSH_KEY_PATH}} -P "${SSH_PORT}" \
        var/tailwind/app.built.css \
        "${SSH_USER}@${SSH_HOST}:${DEPLOY_PATH}/var/tailwind/app.built.css"
    success "app.built.css envoyé"
    ...
    ssh ${SSH_KEY_OPTS} -p "${SSH_PORT}" "${SSH_USER}@${SSH_HOST}" \
        "cd ${DEPLOY_PATH} && \
         ...
         ${PHP_BIN} bin/console asset-map:compile --env=prod && \
