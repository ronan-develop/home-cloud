...
- Vérifié manuellement en navigateur (Playwright, screenshots) : liste filtrée correctement (compteurs cohérents), animation de retrait isolée au bon item, pas de casse du reste de la liste.
- Suite complète : 1146/1146 verts après chaque étape TDD (extension Twig → template/JS/CSS).
...
6. ✅ **FAIT — Nettoyer les notices PHPUnit createMock/createStub** (#272) : 190 → 22 notices dans `tests/Unit/{Entity,EventListener,Security,Service}` (190 → 106 sur la suite complète, 924 tests GREEN). Les `createMock()` sans jamais poser d'`->expects()` remplacés par `createStub()`. Notices résiduelles : mocks partagés en `setUp()` dont au moins un test pose un `expects()` réel sur ce même mock dans un AUTRE test — PHPUnit déclenche la notice par (test, mock) dès que le mock existe sans expectation dans CE test précis, même non utilisé ; les éliminer demanderait d'abandonner le `setUp()` partagé au profit d'une instanciation par test (jugé disproportionné, non fait) — `DefaultFolderServiceTest`, `FolderServiceTest`, `FolderMoverTest`, `SharedResourceCleanerTest`
