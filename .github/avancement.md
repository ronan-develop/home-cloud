...
4. **Rendre les interactions asynchrones (fetch/AJAX) au lieu de POST + reload complet** — ex. suppression d'un Guest fait actuellement un submit PHP classique avec rechargement de page, pas user-friendly ; généraliser le pattern fetch + mise à jour DOM (Stimulus) à toutes les actions de ce type (create/delete/update sans navigation complète)
